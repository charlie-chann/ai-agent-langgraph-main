...
class TestStreamWAL:
    def test_append_and_read_events(self):
        from storage import stream_wal
...
class TestStreamingBatch:
    ...
    async def test_batched_tokens_flush_by_chars(self):
        from core.streaming import batched_tokens
...
class TestModelRouting:
    ...
    @patch("graph.nodes.get_chat_model")
    def test_invoke_chain_uses_aux_role(self, mock_get):
        mock_llm = mock_get.return_value
        with patch("graph.nodes.run_with_timeout", return_value=type("R", (), {"content": '{"blocked": false}'})()):
            from graph.nodes import _invoke_chain
            from prompts.rag_prompts import guard_prompt
